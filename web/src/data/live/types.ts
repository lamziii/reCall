...
/** One author's session notebook (mirrors data/active-session SessionUserNote; inline to avoid a
 *  types cycle). `marks` records timestamp anchors: 'line' (auto, per newline) and 'moment' (the
 *  user's explicit "Mark moment"). */
export interface SessionUserNoteDoc {
  /** Plain-text derivative — kept for backward compat, search, and AI context (clean text, no editor
   *  knowledge). Derived from `doc` via structuredNotesToPlainText on save. */
  content: string
  /** Structured editor content (ProseMirror/Tiptap JSON). Absent on legacy plain-text notes → the UI
   *  migrates on read (plainTextToDoc). */
  doc?: unknown
  format?: 'tiptap' | 'plain'
  marks: { offset?: number; timestamp_seconds: number; kind?: 'line' | 'moment'; created_at_ms?: number }[]
  author_id: string
  workspace_id: string
...
export interface LiveSessionDoc {
  ...
  notes?: string | null
  /** Per-author notebook captured DURING the live session (Active Session / recording dock / PiP
   *  companion), keyed by author id. ONE source of truth — also read/edited by the completed
   *  session's Notes tab. See data/active-session/notes-store.ts (saveSessionNote). */
  user_notes?: Record<string, SessionUserNoteDoc>
  created_by?: string
