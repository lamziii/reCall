...
/**
 * Complete ISO 3166-1 country list (alpha-2 codes). Used by the searchable country selector in
 * onboarding. We store the alpha-2 `code`; the label is display-only. This is a plain data list —
 * no dependency — so the selector never ships a hard-coded five-country dropdown.
 */
...
export function isValidCountryCode(code: string | null | undefined): boolean {
  return Boolean(code && COUNTRY_BY_CODE.has(code.toUpperCase()))
}
